...
Source media is exposed to LibAV through seekable readahead files, so long inputs do not need to play in real time. Edituno does not use the old Safari realtime-audio workaround on the production v0.9.1 path. Windows/Chromium keeps the existing working export path.
