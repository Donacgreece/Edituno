// @ts-nocheck
/* Edituno v2.2.10 editor startup stability production source. TypeScript is canonical; dist is prebuilt for GitHub Pages. */
...
function aboutPage(){
  ...
  $('#app').innerHTML=`<div class="about-page">
    ...
    <main class="about-main">
      <section class="about-hero"><div class="about-hero-copy"><span class="eyebrow">EDITUNO</span><h1>${title}</h1><p>${intro}</p>${installCta?`<div class="about-hero-actions">${installCta}</div>`:''}</div><div class="about-brand-card"><img src="${EDITUNO_ICON}" alt="Edituno"><strong>Edituno</strong><span>${el?'Create locally. Edit freely.':'Create locally. Edit freely.'}</span><div class="about-version">v2.2.10</div></div></section>
      ...
      <footer class="about-footer"><span>Edituno v2.2.10</span><span>${el?'Local-first video editor':'Local-first video editor'}</span></footer>
    </main>
...
function renderHome() {
  ...
  app.innerHTML=`<div class="studio-home ${mobile?'is-mobile':'is-desktop'}">
    <aside class="home-rail desktop-home-only">
      ...
      <div class="home-rail-version">v2.2.10</div>
    </aside>
...
function renderEditor() {
  ...
  app.innerHTML=`<div class="studio-editor ${mobile?'editor-mobile':'editor-desktop'}">
    <header class="editor-topbar">
      <div class="editor-topbar-left"><button class="round-icon" data-action="back" aria-label="${tr('back')}">${svgIcon('back',19)}</button><span class="editor-brand desktop-editor-only">${renderLogo()}</span></div>
    ...
    <main class="editor-workspace">
      ...
      <section class="editor-center">
        <div class="viewer-shell">
          ...
          <div class="preview-zone"><div class="preview-wrap"><div class="preview-frame"><canvas id="preview-canvas"></canvas><div id="preview-interaction-layer" class="preview-interaction-layer ${['clip','overlay','element'].includes(state.selected?.type)?'active':''}" aria-label="Canvas interaction layer"><div id="preview-selection-box" class="preview-selection-box hidden"><i class="selection-handle"></i></div></div><div class="preview-empty ${rows.length?'hidden':''}"><span>${svgIcon('media',25)}</span><strong>${state.language==='el'?'Πρόσθεσε media':'Add media'}</strong><small>${state.language==='el'?'Video, φωτογραφίες ή audio':'Video, photos or audio'}</small><button class="viewer-add" data-action="pick-media">${svgIcon('plus',16)}<span>Import</span></button></div></div></div></div>
...
async function exportProjectLocal(quality,fps,onProgress,signal) {
  const project=state.project; if(!project||(project.clips?.length||0)+(project.overlays?.length||0)+(project.elements?.length||0)===0)throw new Error('empty')
  ...
  try {
    ...
    if(global<total){const rest=total-global;await renderSegment(rest,fps,async elapsed=>{ctx.fillStyle=project.background||'#0b0d12';ctx.fillRect(0,0,w,h);await drawOverlays(ctx,project,global+elapsed,w,h);drawElements(ctx,project,global+elapsed,w,h);drawTexts(ctx,project,global+elapsed,w,h);onProgress((global+elapsed)/total)},signal)}
...
async function init() {
  ...
  try {
    ...
    if('serviceWorker' in navigator && location.protocol.startsWith('http')) {
      const register=()=>navigator.serviceWorker.register('./sw.js?v=2.2.10',{updateViaCache:'none'}).then(reg=>reg.update().catch(()=>{})).catch(error=>console.warn('Service worker registration failed:',error))
