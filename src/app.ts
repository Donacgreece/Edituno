// @ts-nocheck
/* Edituno v2.9.6 LibAV Audio Engine release. TypeScript is canonical; dist is prebuilt for GitHub Pages.
...
function aboutPage(){
  ...
  $('#app').innerHTML=`<div class="about-page">
    ...
    <main class="about-main">
      <section class="about-hero"><div class="about-hero-copy"><span class="eyebrow">EDITUNO</span><h1>${title}</h1><p>${intro}</p>${installCta?`<div class="about-hero-actions">${installCta}</div>`:''}</div><div class="about-brand-card"><img src="${EDITUNO_ICON}" alt="Edituno"><strong>Edituno</strong><span>${el?'Create locally. Edit freely.':'Create locally. Edit freely.'}</span><div class="about-version">v2.9.6</div></div></section>
      ...
      <footer class="about-footer"><span>Edituno v2.9.6</span><span>${el?'Local-first video editor':'Local-first video editor'}</span></footer>
    </main>
...
function renderHome() {
  ...
  app.innerHTML=`<div class="studio-home ${mobile?'is-mobile':'is-desktop'}">
    <aside class="home-rail desktop-home-only">
      ...
      <div class="home-rail-version">v2.9.6</div>
    </aside>
...
async function loadMp4boxModule(){
  if(!mp4boxModulePromise){
    const moduleUrl='./vendor/mp4box.all.mjs?v=2.9.6'
...
async function init() {
  ...
  try {
    ...
    if('serviceWorker' in navigator && location.protocol.startsWith('http')) {
      const register=()=>navigator.serviceWorker.register('./sw.js?v=2.9.6',{updateViaCache:'none'}).then(reg=>reg.update().catch(()=>{})).catch(error=>console.warn('Service worker registration failed:',error))
