// @ts-nocheck
/* Edituno v0.9.7 LibAV Audio Engine release. TypeScript is canonical; dist is prebuilt for GitHub Pages.
...
const STRINGS = {
  en: {
    ...
    speed:'Speed', volume:'Volume', opacity:'Opacity', scale:'Scale', rotation:'Rotation', fit:'Fit', cover:'Cover', contain:'Contain', mirror:'Mirror', flipHorizontal:'Horizontal', flipVertical:'Vertical',
  ...
  el: {
    ...
    speed:'Ταχύτητα', volume:'Ένταση', opacity:'Διαφάνεια', scale:'Μέγεθος', rotation:'Περιστροφή', fit:'Προσαρμογή', cover:'Γέμισμα', contain:'Ολόκληρο', mirror:'Καθρέφτης', flipHorizontal:'Οριζόντια', flipVertical:'Κάθετα',
...
async function probeBlobAudioTrack(blob){
  const M=(window).Mediabunny
  if(!M?.Input||!M?.BlobSource||!M?.ALL_FORMATS)return null
...
async function refreshProjectAudioMetadata(project){
  if(!project)return
  let changed=false
  for(const asset of project.assets||[]){
    ...
    if(!blob)continue
    try{
      const info=await probeBlobAudioTrack(blob)
      if(!info)continue
      for(const key of ['hasAudio','audioCodec','audioSampleRate','audioChannels','audioDecodable']){
        if(asset[key]!==info[key]){asset[key]=info[key];changed=true}
      }
    }catch(error){
      console.warn('Audio metadata probe failed for asset',asset.id,error)
    }
  }
  if(changed){
    project.updatedAt=Date.now()
...
async function mediaMetadata(file) {
  ...
  try {
    ...
    const probe=await probeBlobAudioTrack(file).catch(()=>null)
    const fallbackTracks=type==='video'&&media.audioTracks&&typeof media.audioTracks.length==='number'?media.audioTracks.length:undefined
    const fallbackHasAudio=type==='audio'?true:(fallbackTracks===undefined?undefined:fallbackTracks>0)
    return {
      type,
      duration:Number.isFinite(media.duration)?media.duration:0,
      width:type==='video'?media.videoWidth:undefined,
      height:type==='video'?media.videoHeight:undefined,
      hasAudio:probe?.hasAudio??fallbackHasAudio,
      audioCodec:probe?.audioCodec??null,
      audioSampleRate:probe?.audioSampleRate??null,
      audioChannels:probe?.audioChannels??null,
      audioDecodable:probe?.audioDecodable??null
    }
...
function aboutPage(){
  ...
  $('#app').innerHTML=`<div class="about-page">
    ...
    <main class="about-main">
      <section class="about-hero"><div class="about-hero-copy"><span class="eyebrow">EDITUNO</span><h1>${title}</h1><p>${intro}</p>${installCta?`<div class="about-hero-actions">${installCta}</div>`:''}</div><div class="about-brand-card"><img src="${EDITUNO_ICON}" alt="Edituno"><strong>Edituno</strong><span>${el?'Create locally. Edit freely.':'Create locally. Edit freely.'}</span><div class="about-version">v0.9.7</div></div></section>
      ...
      <footer class="about-footer"><span>Edituno v0.9.7</span><span>${el?'Local-first video editor':'Local-first video editor'}</span></footer>
    </main>
...
function renderHome() {
  ...
  app.innerHTML=`<div class="studio-home ${mobile?'is-mobile':'is-desktop'}">
    <aside class="home-rail desktop-home-only">
      ...
      <div class="home-rail-version">v0.9.7</div>
    </aside>
...
function waitDecodedExportFrame(video,signal,timeout=1200){
  ...
  return new Promise((resolve,reject)=>{
    ...
    video.requestVideoFrameCallback(()=>{if(done)return;done=true;cleanup();resolve(true)})
  })
}
async function settleExportFrameAfterSeek(video,signal){
  if(signal?.aborted)throw new DOMException('Aborted','AbortError')
  if(video.readyState<2)await waitMediaEvent(video,'loadeddata',signal,5000)
  await new Promise((resolve,reject)=>{
    let frames=0,done=false
    const cleanup=()=>signal?.removeEventListener('abort',abort)
    const abort=()=>{if(done)return;done=true;cleanup();reject(new DOMException('Aborted','AbortError'))}
    const tick=()=>{if(done)return;if(++frames>=3){done=true;cleanup();resolve()}else requestAnimationFrame(tick)}
    signal?.addEventListener('abort',abort,{once:true});requestAnimationFrame(tick)
  })
}
async function primeDeterministicVideo(video,signal){
  ...
  try{
    ...
    await video.play()
    await frame
    await settleExportFrameAfterSeek(video,signal)
  }catch(error){
...
function aacFrequencyIndex(sampleRate){
  return [96000,88200,64000,48000,44100,32000,24000,22050,16000,12000,11025,8000,7350].indexOf(Math.round(sampleRate))
}
function aacAudioSpecificConfig(sampleRate=48000,numberOfChannels=2,codec='mp4a.40.2'){
  const parts=String(codec||'').split('.'),objectType=Math.max(1,Math.min(31,Number(parts[2])||2)),index=aacFrequencyIndex(sampleRate),channels=Math.max(1,Math.min(7,numberOfChannels||2))
  if(index<0)return null
  return new Uint8Array([(objectType<<3)|(index>>1),((index&1)<<7)|(channels<<3)])
}
function normalizedAacEncoderMetadata(meta,sampleRate=48000,numberOfChannels=2){
  const description=aacAudioSpecificConfig(sampleRate,numberOfChannels,'mp4a.40.2')
  if(!description)return meta
  return {...(meta||{}),decoderConfig:{...(meta?.decoderConfig||{}),codec:'mp4a.40.2',sampleRate,numberOfChannels,description}}
}
async function loadMp4boxModule(){
  if(!mp4boxModulePromise){
    const moduleUrl='./vendor/mp4box.all.mjs?v=0.9.7'
    mp4boxModulePromise=import(moduleUrl)
  }
  return mp4boxModulePromise
}
async function extractMp4AudioSamples(blob){
  const MP4Box=await loadMp4boxModule(),file=MP4Box.createFile(),buffer=await blob.arrayBuffer()
  buffer.fileStart=0
  return await new Promise((resolve,reject)=>{
    let track=null,samples=[],finished=false
    const timer=setTimeout(()=>finishError(new Error('mp4-audio-extract-timeout')),15000)
    const finishError=error=>{if(finished)return;finished=true;clearTimeout(timer);try{file.stop()}catch{};reject(error)}
    const finishOk=()=>{if(finished)return;finished=true;clearTimeout(timer);try{file.stop()}catch{};resolve({track,samples})}
    file.onError=error=>finishError(new Error(String(error||'mp4-audio-extract')))
    file.onReady=info=>{
      track=(info.audioTracks||[])[0]||(info.tracks||[]).find(item=>item.audio)
      if(!track){finishError(new Error('mp4-no-audio-track'));return}
      file.onSamples=(id,user,batch)=>{
        if(id!==track.id||finished)return
        samples.push(...batch)
        if(track.nb_samples&&samples.length>=track.nb_samples)finishOk()
      }
      file.setExtractionOptions(track.id,null,{nbSamples:Math.max(1,Math.min(512,track.nb_samples||512)),rapAlignement:false})
      file.start()
      if(!track.nb_samples)setTimeout(finishOk,0)
    }
    try{
...
function audioBufferRms(buffer){
  if(!buffer?.length||!buffer.numberOfChannels)return 0
  const step=Math.max(1,Math.floor(buffer.length/240000)),channels=Math.min(2,buffer.numberOfChannels)
  let sum=0,count=0
  for(let c=0;c<channels;c++){
    const data=buffer.getChannelData(c)
    for(let i=0;i<data.length;i+=step){const v=data[i]||0;sum+=v*v;count++}
  }
  return count?Math.sqrt(sum/count):0
}
function clipAllowsSourceAudioPassthrough(clip){
  return Math.abs((clip.speed??1)-1)<.0001&&Math.abs((clip.volume??1)-1)<.0001&&(clip.audioFadeIn||0)<=.0001&&(clip.audioFadeOut||0)<=.0001
}
function projectAllowsSourceAudioPassthrough(project){
  ...
  let audiblePrimary=0
  for(const row of clipTimeline(project)){
    const clip=row.clip,asset=getAsset(clip.assetId,project)
    if(asset?.type!=='video'||asset.hasAudio===false||(clip.volume??1)<=0)continue
    audiblePrimary++
    if(!clipAllowsSourceAudioPassthrough(clip))return false
  }
  return audiblePrimary>0
}
async function prepareSourceAacPassthrough(project){
  if(!projectAllowsSourceAudioPassthrough(project))return null
  const cache=new Map(),segments=[]
  let sampleRate=null,numberOfChannels=null,codec=null,totalSamples=0
  for(const row of clipTimeline(project)){
    const clip=row.clip,asset=getAsset(clip.assetId,project)
    if(asset?.type!=='video'||asset.hasAudio===false||(clip.volume??1)<=0)continue
    let parsed=cache.get(asset.id)
    if(!parsed){
      const blob=await getBlob(asset.id);if(!blob)return null
      try{parsed=await extractMp4AudioSamples(blob)}catch(error){console.warn('AAC passthrough parse failed.',error);return null}
      cache.set(asset.id,parsed)
    }
    const track=parsed?.track,samples=parsed?.samples||[]
    if(!track||!samples.length||!String(track.codec||'').startsWith('mp4a.'))return null
    const sr=track.audio?.sample_rate||track.timescale||48000,ch=Math.max(1,Math.min(2,track.audio?.channel_count||2))
    if(sampleRate===null){sampleRate=sr;numberOfChannels=ch;codec=track.codec}
    if(sr!==sampleRate||ch!==numberOfChannels||track.codec!==codec)return null
    const scale=track.timescale||sr,sourceStart=clip.start||0,sourceEnd=clip.end||asset.duration||0
    const selected=samples.filter(sample=>{
      const pts=(sample.cts??sample.dts??0)/scale,dur=(sample.duration||1024)/scale
      return pts<sourceEnd-.00001&&pts+dur>sourceStart+.00001
    })
    if(!selected.length)return null
    totalSamples+=selected.length
    segments.push({row,clip,asset,track,samples:selected,scale,sourceStart,sourceEnd})
  }
  if(!segments.length||!sampleRate||!numberOfChannels||!totalSamples)return null
  return {sampleRate,numberOfChannels,codec,segments,totalSamples}
}
function muxSourceAacPassthrough(muxer,plan){
  if(!plan?.segments?.length)return 0
  const description=aacAudioSpecificConfig(plan.sampleRate,plan.numberOfChannels,plan.codec)
  if(!description)return 0
  const meta={decoderConfig:{codec:plan.codec,sampleRate:plan.sampleRate,numberOfChannels:plan.numberOfChannels,description}}
  let count=0,lastTimestamp=-1
  for(const segment of plan.segments){
    const {row,samples,scale,sourceStart,sourceEnd}=segment
    for(const sample of samples){
      const pts=(sample.cts??sample.dts??0)/scale,durationSec=(sample.duration||1024)/scale
      let timestampSec=row.start+(pts-sourceStart)
      if(timestampSec<row.start)timestampSec=row.start
      if(timestampSec>=row.end+.03)continue
      let timestamp=Math.max(0,Math.round(timestampSec*1e6))
      if(timestamp<=lastTimestamp)timestamp=lastTimestamp+1
      let duration=Math.max(1,Math.round(Math.min(durationSec,Math.max(.000001,sourceEnd-Math.max(sourceStart,pts)))*1e6))
      const bytes=sample.data instanceof Uint8Array?sample.data:new Uint8Array(sample.data)
      muxer.addAudioChunkRaw(bytes,'key',timestamp,duration,count===0?meta:undefined)
      lastTimestamp=timestamp;count++
    }
  }
  return count
}
async function validateExportAudioEnergy(blob,expectedAudio){
  if(!expectedAudio)return true
  const Ctx=window.OfflineAudioContext
  if(!Ctx||!window.AudioDecoder||!window.EncodedAudioChunk)return null
  try{
    const ctx=new Ctx(2,1,48000),decoded=await decodeMp4AudioWithWebCodecs(blob,ctx)
    if(!decoded)return false
    return audioBufferRms(decoded)>0.000015
  }catch(error){
    console.warn('Export audio energy validation unavailable.',error)
    return null
  }
}

async function decodeMp4AudioWithWebCodecs(blob,ctx){
  const AudioDecoderCtor=window.AudioDecoder,ChunkCtor=window.EncodedAudioChunk
  if(!AudioDecoderCtor||!ChunkCtor||typeof AudioDecoderCtor.isConfigSupported!=='function')return null
  const {track,samples}=await extractMp4AudioSamples(blob)
  if(!track||!samples.length||!String(track.codec||'').startsWith('mp4a.'))return null
  const sampleRate=track.audio?.sample_rate||48000,numberOfChannels=Math.max(1,Math.min(2,track.audio?.channel_count||2))
  const description=aacAudioSpecificConfig(sampleRate,numberOfChannels,track.codec)
  if(!description)return null
  const config={codec:track.codec,sampleRate,numberOfChannels,description}
  const support=await AudioDecoderCtor.isConfigSupported(config).catch(()=>null)
  if(!support?.supported)return null

  const decoded=[]
  let decoderError=null
  const decoder=new AudioDecoderCtor({output:data=>decoded.push(data),error:error=>{decoderError=error}})
  try{
    decoder.configure(config)
    for(let i=0;i<samples.length;i++){
      const sample=samples[i],scale=sample.timescale||track.timescale||sampleRate
      const timestamp=Math.max(0,Math.round((sample.cts??sample.dts??0)/scale*1e6))
      const duration=Math.max(1,Math.round((sample.duration||1024)/scale*1e6))
      decoder.decode(new ChunkCtor({type:'key',timestamp,duration,data:sample.data}))
    ...
    const firstTimestamp=decoded[0].timestamp||0
    const outputSampleRate=decoded[0].sampleRate||sampleRate
    const outputChannels=Math.max(1,Math.min(2,decoded[0].numberOfChannels||numberOfChannels))
    let totalFrames=0
    for(const data of decoded){
      const offset=Math.max(0,Math.round(((data.timestamp||0)-firstTimestamp)/1e6*outputSampleRate))
      totalFrames=Math.max(totalFrames,offset+data.numberOfFrames)
    }
    if(!totalFrames)return null

    const audioBuffer=ctx.createBuffer(outputChannels,totalFrames,outputSampleRate)
    for(const data of decoded){
      const offset=Math.max(0,Math.round(((data.timestamp||0)-firstTimestamp)/1e6*outputSampleRate))
      const frames=Math.min(data.numberOfFrames,totalFrames-offset)
      if(frames<=0){data.close();continue}
      for(let channel=0;channel<outputChannels;channel++){
        const plane=new Float32Array(data.numberOfFrames)
        data.copyTo(plane,{planeIndex:Math.min(channel,(data.numberOfChannels||1)-1),format:'f32-planar'})
        audioBuffer.getChannelData(channel).set(plane.subarray(0,frames),offset)
      }
      data.close()
    }
    return audioBuffer
...
async function decodeOfflineAudio(assetId,ctx,cache){
  ...
  const promise=(async()=>{
    const blob=await getBlob(assetId);if(!blob)return null
    try{
      const decoded=await ctx.decodeAudioData(await blob.arrayBuffer())
      if(decoded?.length)return decoded
    }catch(error){console.warn('Native audio decode failed; trying MP4 WebCodecs demux.',error)}
    try{
      return await decodeMp4AudioWithWebCodecs(blob,ctx)
    }catch(error){
      console.warn('MP4 WebCodecs audio decode failed.',error)
      return null
    }
  })()
...
async function renderOfflineProjectAudio(project,sampleRate=48000){
  ...
  const buffer=hasAudio?await ctx.startRendering():null
  const rms=buffer?audioBufferRms(buffer):0
  return {buffer,hasAudio:hasAudio&&rms>0.000015,expectedAudio,decodeFailures,rms}
}
...
function collectOfflineAudioEvents(project){
  ...
  for(const row of clipTimeline(project)){
    const clip=row.clip,asset=getAsset(clip.assetId,project)
    if(!asset||asset.type!=='video'||asset.hasAudio===false||(clip.volume??1)<=0)continue
    events.push({assetId:asset.id,timelineStart:row.start,duration:row.duration,sourceStart:clip.start||0,speed:clip.speed||1,volume:clip.volume??1,fadeIn:clip.audioFadeIn||0,fadeOut:clip.audioFadeOut||0})
  }
  for(const clip of project.overlays||[]){
    const asset=getAsset(clip.assetId,project)
    if(!asset||asset.type!=='video'||asset.hasAudio===false||(clip.volume??0)<=0)continue
    events.push({assetId:asset.id,timelineStart:clip.timelineStart||0,duration:overlayDuration(clip),sourceStart:clip.start||0,speed:clip.speed||1,volume:clip.volume??0,fadeIn:0,fadeOut:0})
  }
  for(const clip of project.audioClips||[]){
    const asset=getAsset(clip.assetId,project)
    if(!asset||clip.muted||(clip.volume??.8)<=0)continue
    events.push({assetId:asset.id,timelineStart:clip.timelineStart||0,duration:audioClipDuration(clip),sourceStart:clip.sourceStart||0,speed:clip.speed||1,volume:clip.volume??.8,fadeIn:clip.fadeIn||0,fadeOut:clip.fadeOut||0})
  }
  return events.sort((a,b)=>a.timelineStart-b.timelineStart)
}
...
async function ensureOfflineAudioHandles(events){
  ...
  try{
    for(const event of events){
      if(handles.has(event.assetId))continue
      handles.set(event.assetId,await createOfflineAudioHandle(event.assetId))
    }
    return handles
  }catch(error){
    for(const handle of handles.values())try{handle.input.dispose()}catch{}
    throw error
  }
}
function closeOfflineAudioHandles(handles){
  if(!handles)return
  for(const handle of handles.values())try{handle.input.dispose()}catch{}
}
function scheduleOfflineGain(gain,event,globalStart,globalEnd,when,duration){
  const localStart=Math.max(0,globalStart-event.timelineStart),localEnd=Math.max(0,globalEnd-event.timelineStart)
  const g0=eventGainAt(event,localStart),g1=eventGainAt(event,localEnd)
...
async function mixOfflineAudioBlock(project,events,handles,blockStart,blockDuration,sampleRate=48000,fallbackCache=new Map(),signal=null){
  ...
  for(const event of events){
    ...
    const handle=handles.get(event.assetId)
    if(!handle)continue
    ...
    for(const wrapped of wrappedBuffers){
      ...
      const visibleStart=Math.max(timelineStart,bufferTimelineStart),visibleEnd=Math.min(timelineEnd,bufferTimelineEnd)
      if(visibleEnd<=visibleStart+.000001)continue

      const when=visibleStart-blockStart
      const sourceOffset=Math.max(0,(visibleStart-bufferTimelineStart)*speed)
      const sourceDuration=Math.min(Math.max(0,buffer.duration-sourceOffset),Math.max(.000001,(visibleEnd-visibleStart)*speed))
      if(sourceDuration<=.000001)continue

      const node=ctx.createBufferSource(),gain=ctx.createGain()
      node.buffer=buffer;node.playbackRate.value=speed
      node.connect(gain);gain.connect(master)
      scheduleOfflineGain(gain,event,visibleStart,visibleEnd,when,visibleEnd-visibleStart)
      node.start(Math.max(0,when),sourceOffset,sourceDuration)
      scheduled++
    }
  }

  const rendered=await ctx.startRendering()
  return {buffer:rendered,scheduled,rms:audioBufferRms(rendered)}
}
async function exportProjectAppleOfflineChunks(quality,fps,onProgress,signal){
  const project=state.project,M=mediabunnyRuntime(),A=mediabunnyAacRuntime()
  if(!project)throw new Error('empty')
  if(!M?.Output||!M?.Mp4OutputFormat||!M?.BufferTarget||!M?.CanvasSource||!M?.AudioBufferSource||!M?.Quality||!A?.registerAacEncoder)throw new Error('mediabunny-unavailable')

  await refreshProjectAudioMetadata(project)
  const events=collectOfflineAudioEvents(project)
  if(projectExpectsAudio(project)&&!events.length)throw new Error('audio-events-missing')
  const handles=await ensureOfflineAudioHandles(events)
  A.registerAacEncoder()
  ...
  const videoSource=new M.CanvasSource(canvas,{codec:'avc',quality:new M.Quality({bitrate:exportBitrate(quality,fps)})})
  output.addVideoTrack(videoSource,{frameRate:fps})
  const audioSource=new M.AudioBufferSource({
    codec:'aac',fullCodecString:'mp4a.40.2',quality:new M.Quality({bitrate:192000}),
    transform:{numberOfChannels:2,sampleRate:48000}
  })
  output.addAudioTrack(audioSource)
  ...
  try{
    ...
    for(let index=0;index<frameCount;index++){
      ...
      drawElements(ctx,project,time,w,h);drawTexts(ctx,project,time,w,h)
      await videoSource.add(time,actualDuration,{keyFrame:index===0||index%Math.max(1,Math.round(fps*2))===0})
      onProgress((index+1)/frameCount*.68)
    }
    ...
    if(projectExpectsAudio(project)&&audibleBlocks<1)throw new Error('export-silent-audio')
    await output.finalize();onProgress(.98)
    if(!target.buffer||target.buffer.byteLength<4096)throw new Error('offline-export-empty')
    const blob=new Blob([target.buffer],{type:'video/mp4'})
    await validateExportBlob(blob,projectExpectsAudio(project))
    onProgress(1)
    return {blob,extension:'mp4',mime:'video/mp4',audioMode:'offline-chunked-mediabunny'}
  }finally{
    destroyDeterministicSources(sources)
    closeOfflineAudioHandles(handles)
  }
}
...
async function exportProjectWebCodecs(quality,fps,onProgress,signal){
  ...
  const sourceAacPlan=isAppleMobileRuntime()?await prepareSourceAacPassthrough(project):null
  const audioMix=sourceAacPlan?{buffer:null,hasAudio:true,expectedAudio:true,decodeFailures:0,rms:1}:await renderOfflineProjectAudio(project,48000)
  if(audioMix.expectedAudio&&!audioMix.hasAudio)throw new Error('offline-audio-decode')
  const audioConfig=!sourceAacPlan&&audioMix.hasAudio?await chooseAacEncoderConfig(48000,2):null
  if(!sourceAacPlan&&audioMix.hasAudio&&!audioConfig)throw new Error('webcodecs-audio')
  const muxAudio=sourceAacPlan?{codec:'aac',numberOfChannels:sourceAacPlan.numberOfChannels,sampleRate:sourceAacPlan.sampleRate}:audioConfig?{codec:'aac',numberOfChannels:2,sampleRate:48000}:undefined
  const target=new MuxerLib.ArrayBufferTarget()
  const muxer=new MuxerLib.Muxer({target,video:{codec:'avc',width:w,height:h,frameRate:fps},audio:muxAudio,fastStart:'in-memory',firstTimestampBehavior:'strict'})
  ...
  let audioEncoder=null,audioMetadataSent=false
  if(audioConfig&&!sourceAacPlan){
    audioEncoder=new AudioEncoderCtor({
      output:(chunk,meta)=>{
        audioChunks++
        const normalizedMeta=(!audioMetadataSent||meta?.decoderConfig)?normalizedAacEncoderMetadata(meta,48000,2):meta
        muxer.addAudioChunk(chunk,normalizedMeta)
        audioMetadataSent=true
      },
  ...
  try{
    ...
    if(sourceAacPlan){
      audioChunks=muxSourceAacPassthrough(muxer,sourceAacPlan)
      if(audioChunks<1)throw new Error('aac-passthrough-empty')
      onProgress(.96)
    }else if(audioEncoder&&audioMix.buffer){
      await encodeOfflineAudio(audioMix.buffer,audioEncoder,p=>onProgress(.78+p*.18),signal)
    ...
    const blob=new Blob([target.buffer],{type:'video/mp4'});await validateExportBlob(blob,audioMix.expectedAudio)
    if(audioMix.expectedAudio&&isAppleMobileRuntime()){
      const audible=await validateExportAudioEnergy(blob,true)
      if(audible===false)throw new Error('export-silent-audio')
    }
    onProgress(1)
...
async function init() {
  ...
  try {
    ...
    if('serviceWorker' in navigator && location.protocol.startsWith('http')) {
      const register=()=>navigator.serviceWorker.register('./sw.js?v=0.9.7',{updateViaCache:'none'}).then(reg=>reg.update().catch(()=>{})).catch(error=>console.warn('Service worker registration failed:',error))
